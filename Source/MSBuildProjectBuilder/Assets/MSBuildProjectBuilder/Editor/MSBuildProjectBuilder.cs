...
namespace Microsoft.Build.Unity
{
    ...
    public static partial class MSBuildProjectBuilder
    {
        ...
        public static Task<bool> BuildAllProjectsAsync(string arguments = MSBuildProjectBuilder.DefaultBuildArguments)
        {
            return MSBuildProjectBuilder.BuildProjectsAsync(MSBuildProjectBuilder.EnumerateAllMSBuildProjectReferences().ToArray(), arguments);
        }
        ...
        /// <param name="arguments">The additional arguments passed to MSBuild.</param>
        /// <returns>A task that will have a result of true if the build succeeds.</returns>
        public static async Task<bool> BuildProjectsAsync(this IReadOnlyCollection<MSBuildProjectReference> msBuildProjectReferences, string arguments = MSBuildProjectBuilder.DefaultBuildArguments)
        {
            // Sequentialize all builds that show UI for progress.
            await MSBuildProjectBuilder.buildLock.WaitAsync();
            try
            {
                // When in batch mode, simply log the progress.
                if (Application.isBatchMode)
                {
                    return await MSBuildProjectBuilder.BuildProjectsAsync(
            ...
            {
                MSBuildProjectBuilder.buildLock.Release();
        ...
        /// <param name="arguments">The additional arguments passed to MSBuild.</param>
        /// <returns>A task that will have a result of true if the build succeeds.</returns>
        public static Task<bool> BuildProjectAsync(this MSBuildProjectReference mSBuildProjectReference, string arguments = MSBuildProjectBuilder.DefaultBuildArguments)
        {
            return MSBuildProjectBuilder.BuildProjectsAsync(new[] { mSBuildProjectReference }, arguments);
        }
